...
def modify_css_file(css_path, argument):
	...
	if argument == "light":
		...
		if os.path.isdir(css_path) == True:
			if os.path.isfile(os.path.join(css_path, css_dark_file)) == True:
				new_file = open(os.path.join(css_path, css_dark_file), 'r')
				border_width = 0
				final_css_file = open((os.path.join(css_path, css_file)), 'w')
				for line in new_file:
					line = re.sub(r"\${borderWidth}", str(border_width) , str(line))
				...
				final_css_file.close()
				new_file.close()
				os.remove(os.path.join(css_path, css_dark_file))
